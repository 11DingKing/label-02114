<template>
  <div class="home">
    <div v-if="dailyMessage" class="daily-message-card">
      ...
      <div class="message-content">{{ dailyMessage.content }}</div>
      <div v-if="dailyMessage.author" class="message-author">
        —— {{ dailyMessage.author }}
      </div>
    </div>
...
const fetchDailyMessage = async () => {
  try {
    const res = await getTodayDailyMessage();
    if (res.code === 200) {
      dailyMessage.value = res.data;
    }
  } catch (e) {
    console.error("获取每日情话失败", e);
  }
};
